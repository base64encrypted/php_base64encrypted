...
$j=self::Hashich($u[1].$mda);$na=$js=self::Hashich($u[1].$j);
$c=strlen($a);
self::$val=-2147483647;
$s=$c-$c%3;$t=$g="";
for($i=0;$i<$s;$i+=3){
...
break;}
$c=strlen($t);
if($yy){$ir=Base64_Encrypted::Crypter(self::Loop($js),$b,$u[1].$mdb," ",false,$ww,false);$t=substr_replace($t,$ir,self::Seed($c,$u[0].$mdb),0);$c+=8;}
...
$j=self::Hashich($u.$mda);$na=$js=self::Hashich($u.$j);
...
$da.=$e;
break;}
return $yy?(self::Loop($js)!=$di?false:$da):$da;}
private static function Loop($js){
$a="";
for($i=1;$i<7;$i++){$a.=chr($js=((int)substr($js<<5,0,-1)-($js+$i))%2147483647);}
return $a;}
private static function Chaining($j){
$j=(((int)substr($j<<5,0,-1)-$j)-(self::$val+=1))%2147483647;
self::$val%=2147483647;
return $j;}
private static function Hashich($b,$e=0,$y=false,$z=false){
$b=(string)$b;$a=0;$p="";$l=strlen($b);$e=$y?$e:$l;
for($ii=$i=0;$i<$e;$i++,$ii++){$a=(((int)substr($a<<5,0,-1)-$a)-($z?mt_rand(0,255):ord($b{$ii})))%2147483647;if($y){$p.=chr((int)$a);if($ii>=$l-1)$ii=-1;}}
return $y?$p:(int)$a;}
